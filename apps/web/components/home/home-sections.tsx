...
const categoryCards = [
  ...
  },
  {
    ...
    href: "/shop?collection=Everyday%20Grace",
  ...
  },
  {
    ...
    href: "/shop?collection=Festive%20Edit",
...
export function HeroSection() {
  return (
    <section className="section-shell py-6 md:py-10">
      <div className="editorial-panel relative overflow-hidden bg-[color:var(--rukhsar-black)] text-[color:var(--rukhsar-offwhite)]">
        <video
        ...
        <div className="relative px-6 py-8 md:px-10 md:py-12 xl:px-12">
          <div className="grid gap-8 xl:grid-cols-[1.04fr_0.96fr] xl:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                <Link
                  href="/shop?collection=Festive%20Edit"
                  className="cta-secondary border-white/20 bg-white/[0.08] text-white"
                >
                  Shop Now
                </Link>
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {["Premium regional wear", "Fashion-led styling", "Editorial product drops"].map((item) => (
                  <span
                    key={item}
                    className="rounded-full border border-white/12 bg-white/[0.08] px-4 py-2 text-[11px] uppercase tracking-[0.24em] text-white/80"
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {[
                  [
                    "Fast dispatch clarity",
                    "Curated pieces presented with stronger delivery confidence and cleaner expectations."
                  ],
                  [
                    "Craft, not clutter",
                    "Traditional textures are merchandised with stronger hierarchy and room to breathe."
                  ],
                  [
                    "Made for mobile",
                    "The browsing flow stays quick, polished, and easy to trust on smaller screens."
                  ]
                ].map(([title, body]) => (
                  <div key={title} className="rounded-[1.6rem] border border-white/12 bg-white/[0.08] p-5 backdrop-blur-md">
                    <p className="font-semibold uppercase tracking-[0.16em] text-[color:var(--rukhsar-pink-soft)]">
                      {title}
                    </p>
            ...
            <div className="grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                {[
                  [
                    "Pan-India delivery",
                    "Clear shopping pathways and premium presentation designed to feel dependable."
                  ],
                  [
                    "Secure payment feel",
                    "Checkout, support, and tracking flows that read calmer and more trustworthy."
                  ]
                ].map(([title, body]) => (
                  <div
                    key={title}
                    className="rounded-[1.6rem] border border-white/10 bg-white/[0.08] p-5 backdrop-blur-md"
                  >
                    <p className="text-[11px] uppercase tracking-[0.3em] text-[color:var(--rukhsar-pink-soft)]">
                      {title}
                    </p>
              ...
              <div className="w-full rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.16),rgba(255,255,255,0.04))] p-6 backdrop-blur-xl">
                ...
                <p className="mt-5 text-sm leading-7 text-white/72 md:text-base">
                  Think heirloom color, shimmer that reads premium, and silhouettes that feel right for intimate
                  nights, wedding scenes, and statement festive dressing.
                </p>
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <span className="fashion-chip bg-[color:var(--rukhsar-mustard)] text-[color:var(--rukhsar-black)]">
                    Occasion ready
                  </span>
                </div>
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link href="/shop?collection=Festive%20Edit" className="cta-primary">
                    Shop the edit
                  </Link>
                  <Link
                    href="/shop?sort=price-desc"
                    className="rounded-full border border-white/16 bg-white/[0.08] px-6 py-3 text-sm font-semibold text-white"
                  >
                    Premium picks
                  </Link>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                {[
                  ["Loved for festive styling", "Built around standout color and stronger occasion dressing."],
                  ["Curated for easy discovery", "Less clutter, clearer pathways, and faster product scanning."],
                  ["Support-led experience", "Cart, checkout, and tracking flows that feel calmer and more trustworthy."]
                ].map(([title, body]) => (
                  <div key={title} className="rounded-[1.6rem] border border-white/10 bg-white/[0.08] p-5 backdrop-blur-md">
                    <p className="text-[11px] uppercase tracking-[0.24em] text-white/80">{title}</p>
                    <p className="mt-3 text-sm leading-7 text-white/70">{body}</p>
                  </div>
                ))}
              </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-5">
        {categoryCards.map((card) => (
          <Link
            key={card.title}
            href={card.href}
            className={`rounded-[1.8rem] p-5 shadow-[0_20px_40px_rgba(17,11,18,0.12)] transition hover:-translate-y-1 ${card.accent}`}
          >
            ...
            <p className="mt-3 text-sm leading-7 opacity-90">{card.subtitle}</p>
          </Link>
        ))}
      </div>
    ...
    <section className="section-shell py-10">
      <div className="rounded-[2.1rem] bg-[linear-gradient(135deg,rgba(15,11,18,1),rgba(42,17,31,0.96)_55%,rgba(97,31,48,0.92))] px-6 py-8 text-white shadow-[0_28px_70px_rgba(17,11,18,0.24)] md:px-10 md:py-10">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <SectionHeading
            title="Best Sellers"
            eyebrow="Gen-Z Fashion Spotlight"
            className="text-white [&_p]:text-white/72 [&_h2]:text-white"
          >
  ...
  return (
    <section className="section-shell py-10">
      <div className="rounded-[2rem] bg-[linear-gradient(135deg,rgba(255,79,163,0.92),rgba(108,31,46,0.94),rgba(15,11,18,1))] px-6 py-8 text-white shadow-[0_24px_64px_rgba(108,31,46,0.24)] md:px-10 md:py-10">
        <SectionHeading
          title="Stay close to every new drop"
          eyebrow="Newsletter"
